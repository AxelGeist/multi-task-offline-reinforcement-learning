...
def optimize_bc():
    ...
    fig.update_layout(title=f"History: BC Tuning on {dataset_size}x {dataset_quality.capitalize()} ({dataset_pct}%) Dataset",)
    fig.write_image(f"results/tuning/{study_name}_{dataset_quality}_history.png") 
    fig = plot_rank(study)
    fig.update_layout(title=f"Rank: BC Tuning on {dataset_size}x {dataset_quality.capitalize()} ({dataset_pct}%) Dataset",)
    ...
    fig.update_layout(title=f"Contour: BC Tuning on {dataset_size}x {dataset_quality.capitalize()} ({dataset_pct}%) Dataset",)
