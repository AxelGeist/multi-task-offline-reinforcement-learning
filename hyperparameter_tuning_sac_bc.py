...
def objective(trial):
    ...
    df_rewards = discrete_SAC_BC.eval(config=config, model_paths=model_path)
...
def optimize_sac_bc():
    ...
    storage_name = "sqlite:///tuning_sac_bc.db".format(study_name)
    ...
    fig.update_layout(title="History: SAC+BC Tuning on 40x Optimal (100%) Dataset",)
    fig.write_image(f"results/{study_name}_history.png") 
    fig = plot_rank(study)
    fig.update_layout(title="Rank: SAC+BC Tuning on 40x Optimal (100%) Dataset",)
    fig.write_image(f"results/{study_name}_rank.png")
    fig = plot_contour(study, params=[ "learning_rate", "n_steps", "gamma"])
    fig.update_layout(title="Contour: SAC+BC Tuning on 40x Optimal (100%) Dataset",)
